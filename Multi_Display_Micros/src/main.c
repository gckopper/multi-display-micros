...
void TIM1_UP_TIM10_IRQHandler (void) // Quando o bit UIF de TIM10 virar 1
{
	if (frase != NULL) { // Verifica se o vetor frase é não nulo, pois caso ele seja nulo, não haverá mensagem para ser mostrada
		...
		if (*frase == '\0') // Caso o seja o último caractere do espaço reservado para a frase
		{
			frase -= (tamanho - 1); // Retorna para o primeiro endereço da frase usando o tamanho dela;
		...
		{
			frase -= nDisplays; // Retorna nDisplays endereços no espaço da frase (para ir para o caractere do primeiro display)
		...
		frase++; // Passa para o próximo caractere da frase
...
*/
	...
	if (*fEntra == '\0' || *dPin == '\0' || dEntra == 0 || lTime == 0 || (selectMode != 'u' && selectMode != 'c')) // Caso não esteja bem configurada a função
	{
	...
	lTempo = lTime - lTime % dEntra; // Passando o tempo configurado para a variável global
